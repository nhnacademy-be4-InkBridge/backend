...
class OrderAdminControllerTest {
    ...
    void testGetOrderList() throws Exception {
        ...
        mockMvc.perform(get("/api/admin/orders")
            ...
            .andExpectAll(
                status().isOk(),
                jsonPath("$.content[0].orderId").value(1L),
                jsonPath("$.content[0].orderCode").value("orderCode"),
                jsonPath("$.content[0].orderName").value("orderName"),
                jsonPath("$.content[0].orderAt").value(
                    DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss")
                        .format(LocalDateTime.of(2024, 1, 1, 0, 0, 0))),
                jsonPath("$.content[0].deliveryDate").value(
                    DateTimeFormatter.ofPattern("yyyy-MM-dd").format(LocalDate.of(2024, 1, 1))),
                jsonPath("$.content[0].totalPrice").value(30000L)
            )
    ...
    void testGetOrderDetailByOrderId_success() throws Exception {
        ...
        mockMvc.perform(RestDocumentationRequestBuilders.get("/api/admin/orders/{orderId}", 1L)
            ...
            .andExpectAll(
                ...
                jsonPath("$.orderInfo.deliveryDate").value(
                    DateTimeFormatter.ofPattern("yyyy-MM-dd").format(LocalDate.of(2024, 1, 1))),
